...
status: open
description: >
  19 ruled the calibre password low-value on the reading that it was one
  credential in one place. 41 found the same string is qbittorrent's WebUI
  password, lazylibrarian's own UI password, and the *arr's download-client
  credential. Decide whether that ruling survives.
touches: []
---
